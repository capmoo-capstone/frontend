  ...
  if (!submissions || submissions.length === 0) {
    return (
      <div className="text-muted-foreground bg-muted/20 flex h-48 flex-col items-center justify-center rounded-lg border border-dashed">
        ...
        <span className="text-sm">ยังไม่มีประวัติการส่งงาน</span>
      </div>
    );
  }
            ...
            <div className="flex flex-row items-start justify-between">
              <div className="flex flex-col space-y-1">
                <span className="text-base font-medium">
                  ส่งครั้งที่ {submission.submission_round}
                </span>
              ...
              <>
                ...
                <div className="flex flex-col items-start gap-1">
                  <div className="flex w-full flex-row items-center justify-between gap-2">
                    ...
                    <span className="flex-1 text-base font-medium">ถูกตีกลับ</span>
                    <span className="text-muted-foreground text-sm">
                      {formatDateThai(submission.action_at, 'd MMM yyyy HH:mm น.')}
                    </span>
                  </div>
                  <p className="ml-6 text-base">"{submission.comments}"</p>
            ...
            {['ACCEPTED', 'APPROVED'].includes(submission.status) && (
              <>
                ...
                <div className="flex flex-col items-start gap-1">
                  <div className="flex w-full flex-row items-center justify-between gap-2">
                    <CornerDownRight className="text-primary h-4 w-4" />
                    <span className="flex-1 text-base font-medium">อนุมัติ</span>
                    <span className="text-muted-foreground text-sm">
                      {formatDateThai(submission.action_at, 'd MMM yyyy HH:mm น.')}
                    </span>
                  </div>
            ...
            {submission.status === 'APPROVED' && (
              <>
                ...
                <div className="flex flex-col items-start gap-1">
                  <div className="flex w-full flex-row items-center justify-between gap-2">
                    <CornerDownRight className="text-primary h-4 w-4" />
                    <span className="flex-1 text-base font-medium">ผู้บริหารลงนามฯ</span>
                    <span className="text-muted-foreground text-sm">
                      {formatDateThai(submission.action_at, 'd MMM yyyy HH:mm น.')}
                    </span>
                  </div>
